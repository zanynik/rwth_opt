...
$\newline$
\begin{align}
\sum_{m \in M}\sum_{t=a_{l}}^{d_l} z_{lkm}^{t} = 1  \qquad \forall l \in L_k, \forall k \in K 
\end{align}
...
\begin{align}
Q_{mk}^{t}.W - \sum_{l\in L_k}z_{lkm}^{t}.w_l \ge  \sum_{p\in L_k\setminus \{ l\}}            \sum_{\tau=t+1}^{t+b_k-1} z_{pkm}^{\tau}.w_l , \qquad \forall m \in M, \forall k \in K 
\end{align}
...
$\newline$
\begin{align}
1 - {Q_{mk}^{t}} \ge \sum_{\tau = t}^{t + b_{k}}{Q_{mq}^{t}} ,  \qquad \forall q \in K \setminus \{k\} , \forall m \in M, \forall t \in T
\end{align}
...
\begin{align}
 \sum_{j \in J}\sum_{t=a_{l}}^{d_{i}}{\color{black}{(t + h_{j} + T_{j})}} . y_{lij}^t  \le \sum_{m \in M_{k}}\sum_{t=a_{l}}^{d_{l}}z_{lkm}^t . t \qquad \forall i \in I , \forall  \forall l \in L_{i}, \forall k \in K, \forall t \in T
\end{align}
