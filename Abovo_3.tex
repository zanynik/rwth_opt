...
Tha above equation ensures that the capacity of a BU zone is not violated. The same equation can be simplifed if we initially discretize time in a coarse manner i.e instead of discretizing time in minutes, if we discretize time in handling time intervals of the BU zone and define the decision variable only at discretized time values, then we can rewrite the equations as follows:

\begin{align}
Q_{mk}^{t}.W - \sum_{l\in L_k}z_{lm}^{t}.w_l \ge 0 \qquad \forall t \in T, \forall j \in J
\end{align}
